...
사용:
  python privilege_revoke_bot.py                       # 1회 탐지 + Graylog 신고
  python privilege_revoke_bot.py --dry-run             # 신고 없이 위반만 출력
  python privilege_revoke_bot.py --revoke               # (대체) 게시판 API 로 직접 회수까지
  python privilege_revoke_bot.py --interval 60         # 60초마다 반복 실행(주기적 감시)
  python privilege_revoke_bot.py --revoke --interval 10 # 10초마다 자동 탐지 및 직접 회수
"""
import argparse
...
def run_once(c, args):
  try:
    ...
    print(f'[{datetime.now().strftime("%H:%M:%S")}] [!] 게시판 조회 실패: {e}')
    return

  now_str = datetime.now().strftime("%H:%M:%S")
  if not bad:
    print(f"[{now_str}] [OK] 과잉권한 위반 없음 (허용목록: {c['allow'] or '(비어있음=모든 admin이 위반)'})")
    return

  print(f"[{now_str}] [!] 과잉권한 admin {len(bad)}건 탐지: " + ', '.join(u['username'] for u in bad))
  for u in bad:
    if args.dry_run:
...
def main():
  ...
  ap.add_argument('--interval', type=int, default=0, help='반복 실행 주기(초). 지정하면 해당 초마다 계속 감시 (예: 60)')
  args = ap.parse_args()
  ...
  if args.interval > 0:
    mode_desc = "탐지 + 직접 회수" if args.revoke else ("dry-run(출력만)" if args.dry_run else "탐지 + Graylog 신고")
    print(f"[*] 권한 감시봇 상시 실행 시작 (주기: {args.interval}초, 동작: {mode_desc}, 종료: Ctrl+C)")
    try:
      while True:
        run_once(c, args)
